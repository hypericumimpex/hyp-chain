...
if ( ! class_exists( 'WC_Chained_Products' ) ) {
	...
	class WC_Chained_Products {
		...
		public function woocommerce_chained_add_to_cart_validation( $add_to_cart, $product_id, $main_product_quantity ) {
			...
			if ( isset( $_GET['order_again'] ) && is_user_logged_in() && isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'woocommerce-order_again' ) ) {  // WPCS: sanitization ok, input var ok.
				$order = wc_get_order( absint( $_GET['order_again'] ) ); // WPCS: input var ok.
			...
			if ( isset( $_GET['resubscribe'] ) && isset( $_GET['_wpnonce'] ) ) { // WPCS: input var ok.
				$subscription = wcs_get_subscription( $_GET['resubscribe'] );  // WPCS: sanitization ok, input var ok.
			...
			$product_id = ( isset( $_REQUEST['variation_id'] ) && $_REQUEST['variation_id'] > 0 ) ? $_REQUEST['variation_id'] : $product_id;  // WPCS: sanitization ok, input var ok.
		...
		public function get_chained_products_html_view( $chained_attributes ) {
			...
			if ( isset( $_POST['form_value']['variable_id'] ) && null !== $_POST['form_value']['variable_id'] ) { // WPCS: CSRF ok, input var ok.

				$chained_parent_id    = wc_clean( wp_unslash( $_POST['form_value']['variable_id'] ) ); // WPCS: sanitization ok, CSRF ok, input var ok.
				$shortcode_attributes = $_POST['form_value']; // WPCS: sanitization ok, CSRF openssl_free_key(key_identifier), input var ok.
