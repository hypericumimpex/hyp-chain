...
if ( ! class_exists( 'WC_CP_Admin_Notices' ) ) {
	...
	class WC_CP_Admin_Notices {
		...
		public function cp_dismiss_admin_notice() {
			if ( isset( $_GET['cp_dismiss_admin_notice'] ) && '1' === $_GET['cp_dismiss_admin_notice'] && isset( $_GET['option_name'] ) ) { // WPCS: CSRF ok, input var ok.
				$option_name = sanitize_text_field( wp_unslash( $_GET['option_name'] ) ); // WPCS: CSRF ok, input var ok.
		...
		public function sa_cp_show_review_notice() {
			global $post;
			$is_review_page = ( isset( $post->post_type ) && ( 'shop_order' === $post->post_type || 'product' === $post->post_type ) ) ? true : false; // WPCS: CSRF ok.
		...
		public function sa_cp_update_notice_action() {
			$action = ( isset( $_GET['cp_notice_action'] ) ) ? wc_clean( wp_unslash( $_GET['cp_notice_action'] ) ) : ''; // WPCS: CSRF ok, sanitization ok, input var ok.
