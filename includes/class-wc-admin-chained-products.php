...
if ( ! class_exists( 'WC_Admin_Chained_Products' ) ) {
	...
	class WC_Admin_Chained_Products {
		...
		public function exclude_parent_variable_product_from_search() {
			$search_result = ( isset( $_POST['result'] ) ) ? $_POST['result'] : ''; // WPCS: CSRF ok, sanitization ok, input var ok.
		...
		public function add_order_item_meta_manually( $item_id, $item ) {
			if ( $this->is_wc_gte_30() ) {
				$order_id    = ! empty( $_POST['order_id'] ) ? absint( $_POST['order_id'] ) : ''; // WPCS: CSRF ok, input var ok.
				...
				$chained_product_details = ! empty( $_POST['chained_product_details'] ) ? $_POST['chained_product_details'] : ''; // WPCS: CSRF ok, sanitization ok, input var ok.
		...
		public function on_add_order_item_manually() {
			...
			if ( $this->is_wc_gte_35() ) {
				$item_to_add = ( ! empty( $_POST['data'] ) ) ? array_filter( wp_unslash( (array) $_POST['data'] ) ) : array(); // WPCS: sanitization ok, input var ok.
			} else {
				$item_to_add = $this->is_wc_gte_30() ? ( ( ! empty( $_POST['item_to_add'] ) && is_array( $_POST['item_to_add'] ) ) ? wp_parse_id_list( $_POST['item_to_add'] ) : wp_parse_id_list( array( $_POST['item_to_add'] ) ) ) : absint( $_POST['item_to_add'] ); // WPCS: sanitization ok, input var ok.
			}

			$order_id = ! empty( $_POST['order_id'] ) ? absint( $_POST['order_id'] ) : ''; // WPCS: input var ok.
			$order    = wc_get_order( $order_id );
		...
		public function add_chained_products_in_given_order( $order_id = 0, $post = null, $update = false ) {
			...
			if ( in_array( 'woocommerce-give-products/woocommerce-give-products.php', $active_plugins, true ) || array_key_exists( 'woocommerce-give-products/woocommerce-give-products.php', $active_plugins ) ) {
				if ( empty( $_GET['page'] ) || 'give_products' !== $_GET['page'] ) { // WPCS: CSRF ok, input var ok.
					return;
				}
				if ( empty( $_GET['give_products_nonce'] ) || ! wp_verify_nonce( wp_unslash( $_GET['give_products_nonce'], 'give_products' ) ) ) { // WPCS: sanitization ok, CSRF ok, input var ok.
					return;
				}
				if ( empty( $order_id ) ) {
					return;
		...
		public function cp_save_product_variations( $product_id ) {
			...
			$variable_product_ids = ! empty( $_POST['variable_post_id'] ) ? $_POST['variable_post_id'] : array(); // WPCS: sanitization ok, input var ok.
		...
		public function on_process_product_meta( $post_id, $post ) {
			...
			if ( empty( $_POST['woocommerce_meta_nonce'] ) || ! wp_verify_nonce( wp_unslash( $_POST['woocommerce_meta_nonce'] ), 'woocommerce_save_data' ) ) { // WPCS: sanitization ok, input var ok.
				return;
			}
		...
		public function update_chained_products_order( $chained_parent_id ) {
			...
			*/
			...
			if ( ! empty( $order_with_product_details ) ) {
				...
				*/
				$all_chained_products_ids    = ( ! empty( $_POST['chained_products_ids'] ) ) ? $_POST['chained_products_ids'] : array(); // WPCS: CSRF ok, sanitization ok, input var ok.
				$nested_chained_products_ids = ( ! empty( $_POST['nested_chained_products_ids'] ) ) ? $_POST['nested_chained_products_ids'] : array(); // WPCS: CSRF ok, sanitization ok, input var ok.
					...
					if ( ! empty( $items ) ) {
						...
						foreach ( $items as $item_id => $item ) {
							...
							if ( ! empty( $all_chained_products_ids[ $chained_parent_id ] ) && in_array( $product_id, $all_chained_products_ids[ $chained_parent_id ], true ) ) {
								...
								$unit                        = ( ! empty( $_POST['chained_products_quantity'][ $chained_parent_id ][ $index ] ) ) ? wc_clean( wp_unslash( $_POST['chained_products_quantity'][ $chained_parent_id ][ $index ] ) ) : 1; // WPCS: CSRF ok, sanitization ok, input var ok.
								...
								$new_qty                     = (int) ( $chained_parent_qty_in_order * $unit );
								$old_qty                     = (int) ( ( ! empty( $item['_qty'] ) ) ? $item['_qty'] : 1 );
								if ( $new_qty !== $old_qty ) {
		...
		public function remove_shortcode_from_post_content( $data ) {
			if ( isset( $_POST['post_type'] ) && 'product' === wc_clean( wp_unslash( $_POST['post_type'] ) ) ) { // WPCS: sanitization ok, input var ok.

				$product_id = ! empty( $_POST['ID'] ) ? absint( $_POST['ID'] ) : ''; // WPCS: CSRF ok, input var ok.
				$product    = wc_get_product( $product_id );
		...
		public function update_chained_product_data( $chained_parent_id ) {
			if ( isset( $_POST['chained_products_ids'][ $chained_parent_id ] ) && ! empty( $_POST['chained_products_ids'][ $chained_parent_id ] ) ) { // WPCS: input var ok.

				$chained_products_ids = array_filter( $_POST['chained_products_ids'][ $chained_parent_id ] ); // WPCS: CSRF ok, sanitization ok, input var ok.

				if ( empty( $chained_products_ids ) ) {
					return;
				}

				$chained_products_quantity            = ! empty( $_POST['chained_products_quantity'][ $chained_parent_id ] ) ? $_POST['chained_products_quantity'][ $chained_parent_id ] : 1; // WPCS: CSRF ok, sanitization ok, input var ok.
				$chained_products_priced_individually = ! empty( $_POST['chained_products_priced_individually'][ $chained_parent_id ] ) ? $_POST['chained_products_priced_individually'][ $chained_parent_id ] : ''; // WPCS: CSRF ok, sanitization ok, input var ok.
				...
				if ( ! empty( $_POST['product-type'] ) && ( 'variable-subscription' === $_POST['product-type'] || 'subscription' === $_POST['product-type'] ) ) { // WPCS: CSRF ok, input var ok.
				...
				$post_product_type = wp_unslash( $_POST['product-type'] ); // WPCS: CSRF ok, sanitization ok, input var ok.
				...
				if ( 'yes' === get_option( 'woocommerce_manage_stock' ) ) {
					...
					if ( isset( $_POST['chained_products_manage_stock'][ $chained_parent_id ] ) && 'on' === wc_clean( wp_unslash( $_POST['chained_products_manage_stock'][ $chained_parent_id ] ) ) ) { // WPCS: CSRF ok, sanitization ok, input var ok.
		...
		public function cp_styles_and_scripts() {
			if ( ! empty( $_GET['tab'] ) && ! empty( $_GET['section'] ) && 'products' === $_GET['tab'] && 'wc_chained_products' === $_GET['section'] ) { // WPCS: CSRF ok, input var ok.
